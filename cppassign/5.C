...
class matrix
{
	...
	void read_matrix();
	void print_matrix();
	matrix operator-(matrix m2)//m3=m1-m2;
	{
		matrix m3;
		for(int i=0;i<r;i++)
			for(int j=0;j<c;j++)
	...
	friend matrix operator+(matrix,matrix);//m3=m1+m2;
	friend matrix operator+(int,matrix);//m3=4+m2;
	matrix operator+(int x)//m3=m1+3;
	{
		for(int i=0;i<r;i++)
		for(int j=0;j<c;j++)
			a[i][j]=a[i][j]+x;//(or)this->a[i][j]=this->a[i][j]+x;
		return (*this);
		/*matrix m;
		for(int i=0;i<r;i++)
		for(int j=0;j<c;j++)
